...
LRESULT CMainDlg::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
{
	...
	else
	{
		...
		if (IsWin8OrLater())
			...
			SetDlgItemText(IDC_ENTRYNAME, (m_ExistingEntryName + _T(" [VKD-Redux]")).c_str());
	}
...
LRESULT CMainDlg::OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	else
	{
		...
		{
			if (IsWin8OrLater())
				MessageBox(_T("Warning: You are using Windows 8 or later. You will have to disable driver signature enforcement MANUALLY at every boot to use VirtualKD-Redux! Please refer to http://virtualkd.sysprogs.org/win8 for more details!"), _T("Warning"), MB_ICONWARNING);
